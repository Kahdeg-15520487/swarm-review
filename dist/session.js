 ...
 */
import { AuthStorage, ModelRegistry, createAgentSession, SessionManager, DefaultResourceLoader, defineTool, getAgentDir, } from "@earendil-works/pi-coding-agent";
import { Type } from "typebox";
function createReportFindingTool(category) {
    const findings = [];
...
export async function createReviewerSession(options) {
    ...
    const reportTool = isCoordinator
        ...
        : createReportFindingTool(category);
...
export async function runSession(session, prompt, timeoutMs, signal) {
    const output = [];
    const usage = {
        inputTokens: 0,
        outputTokens: 0,
