...
function buildCoordinatorPrompt(reviewerResults, diffResult, config) {
    const findingsXml = reviewerResults
        ...
        const findings = r.findings
            .map((f) => `  <finding severity="${f.severity}" category="${f.category}">\n` +
            `    <title>${f.title}</title>\n` +
...
export async function runCoordinator(reviewerResults, diffResult, riskTier, config, signal) {
    const startTime = Date.now();
    const totalUsage = reviewerResults.reduce((acc, r) => ({
    ...
    try {
        ...
        const { usage: coordinatorUsage } = await runSession(session, prompt, coordinatorTimeout, signal);
        ...
        const verdict = review?.verdict ?? deriveVerdict(reviewerResults);
        return {
            verdict,
...
function deriveVerdict(reviewers) {
